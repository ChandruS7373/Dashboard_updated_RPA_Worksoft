    ...
    if _dept_tab_rpa is not None:
      with _dept_tab_rpa:
        ...
        if role in ("lead", "manager"):
            tab_mine_rpa, tab_dev, tab_rm, tab_completed, tab_uat, tab_disc, tab_bot_metrics = st.tabs([
                "My Projects", "Development", "R&M", "Completed", "UAT", "Discontinued", "⏱️ Bot Metrics",
            ])
        else:
            tab_mine_rpa = None
            tab_dev, tab_rm, tab_completed, tab_uat, tab_disc, tab_bot_metrics = st.tabs([
                "Development", "R&M", "Completed", "UAT", "Discontinued", "⏱️ Bot Metrics",
            ])
        ...
        with tab_bot_metrics:
            # ── Ensure columns exist ──────────────────────────────────────────────
            for _bmc in ["num_bots", "manual_run_mins", "bot_run_mins", "num_persons"]:
                if _bmc not in st.session_state.projects.columns:
                    st.session_state.projects[_bmc] = 0

            _bm_df = _rpa_base.copy()
            for _bmc in ["num_bots", "manual_run_mins", "bot_run_mins", "num_persons"]:
                if _bmc not in _bm_df.columns:
                    _bm_df[_bmc] = 0
                _bm_df[_bmc] = pd.to_numeric(_bm_df[_bmc], errors="coerce").fillna(0)

            # Current-month logs for KPI
            _bm_month_start = date.today().replace(day=1).isoformat()
            _bm_month_end   = date.today().isoformat()
            _bm_cur_logs    = auth.get_bot_metric_logs(start_date=_bm_month_start, end_date=_bm_month_end)
            _bm_qty_map: dict = {}
            for _lg in _bm_cur_logs:
                _lpid = int(_lg.get("project_id", 0))
                _bm_qty_map[_lpid] = _bm_qty_map.get(_lpid, 0) + int(_lg.get("qty", 0) or 0)

            _bm_total_bots    = int(_bm_df["num_bots"].sum())
            _bm_total_persons = int(_bm_df["num_persons"].sum())
            _bm_proj_tracked  = int((_bm_df["num_bots"] > 0).sum())
            _bm_total_saved   = 0.0
            for _, _kr in _bm_df.iterrows():
                _kpid = int(float(_kr.get("id", 0) or 0))
                _kqty = _bm_qty_map.get(_kpid, 0)
                _bm_total_saved += max(
                    float(_kr.get("manual_run_mins", 0) or 0) * float(_kr.get("num_persons", 0) or 0)
                    - float(_kr.get("bot_run_mins", 0) or 0) * float(_kr.get("num_bots", 0) or 0), 0
                ) * _kqty / 60

            # ── KPI strip ─────────────────────────────────────────────────────────
            _bk1, _bk2, _bk3, _bk4 = st.columns(4)
            _bk1.metric("Total Active Bots",      _bm_total_bots)
            _bk2.metric("Total Persons (Manual)", _bm_total_persons)
            _bk3.metric("This Month Saved",       f"{_bm_total_saved:,.1f} hrs")
            _bk4.metric("Projects Configured",    _bm_proj_tracked)

            st.markdown("<br>", unsafe_allow_html=True)

            # ── Per-project cards ─────────────────────────────────────────────────
            if _bm_df.empty:
                st.info("No RPA projects found.")
            else:
                for _bm_i, _bm_r in _bm_df.iterrows():
                    _bm_proj_name   = str(_bm_r.get("name", "") or "")
                    _bm_proj_client = str(_bm_r.get("client", "") or "")
                    _bm_proj_status = str(_bm_r.get("status", "") or "")
                    _bm_pid         = int(float(_bm_r.get("id", 0) or 0))
                    _bm_has_metrics = float(_bm_r.get("num_bots", 0) or 0) > 0
                    _bm_month_qty   = _bm_qty_map.get(_bm_pid, 0)
                    _bm_saved_disp  = max(
                        float(_bm_r.get("manual_run_mins", 0) or 0) * float(_bm_r.get("num_persons", 0) or 0)
                        - float(_bm_r.get("bot_run_mins", 0) or 0) * float(_bm_r.get("num_bots", 0) or 0), 0
                    ) * _bm_month_qty / 60

                    _exp_label = (
                        f"**{_bm_proj_name}**  ·  {_bm_proj_client}  ·  {_bm_proj_status}"
                        + (f"  ✅  {_bm_saved_disp:.1f} hrs saved this month"
                           if _bm_has_metrics and _bm_month_qty > 0
                           else ("  ⚙️ Configured" if _bm_has_metrics else "  ⚪ Not configured"))
                    )
                    with st.expander(_exp_label, expanded=not _bm_has_metrics):
                        _bk = f"bm_{_bm_i}"
                        if role in ("admin", "lead", "manager"):
                            # ── Settings ─────────────────────────────────────────
                            st.markdown(
                                '<div style="font-size:11px;font-weight:700;color:#1F3B4D;'
                                'margin-bottom:6px">⚙️ Bot Settings</div>',
                                unsafe_allow_html=True
                            )
                            _pc1, _pc2, _pc3, _pc4 = st.columns(4)
                            _inp_np = _pc1.number_input("No. of Persons",        min_value=0,   value=int(float(_bm_r.get("num_persons",    0) or 0)), step=1,   key=f"{_bk}_np")
                            _inp_nb = _pc2.number_input("No. of Bots",           min_value=0,   value=int(float(_bm_r.get("num_bots",        0) or 0)), step=1,   key=f"{_bk}_nb")
                            _inp_mr = _pc3.number_input("Manual Run Time (mins)", min_value=0.0, value=float(_bm_r.get("manual_run_mins",     0) or 0),  step=1.0, key=f"{_bk}_mr")
                            _inp_br = _pc4.number_input("Bot Run Time (mins)",    min_value=0.0, value=float(_bm_r.get("bot_run_mins",        0) or 0),  step=1.0, key=f"{_bk}_br")
                            if st.button("💾 Save Settings", type="primary", key=f"{_bk}_save"):
                                _pi = st.session_state.projects[
                                    st.session_state.projects["name"] == _bm_proj_name
                                ].index
                                if not _pi.empty:
                                    st.session_state.projects.loc[_pi, "num_persons"]     = str(_inp_np)
                                    st.session_state.projects.loc[_pi, "num_bots"]        = str(_inp_nb)
                                    st.session_state.projects.loc[_pi, "manual_run_mins"] = str(_inp_mr)
                                    st.session_state.projects.loc[_pi, "bot_run_mins"]    = str(_inp_br)
                                    save_projects_async(st.session_state.projects)
                                    auth.log_audit(cu["id"], cu["name"], "UPDATE", "projects",
                                                   str(_bm_r.get("id", "")),
                                                   f'Bot settings updated for "{_bm_proj_name}"')
                                    st.session_state.toast = {"msg": f"Settings saved for {_bm_proj_name}!", "type": "success"}
                                    st.rerun()
                            ...
                            st.markdown(
                                '<div style="font-size:11px;font-weight:700;color:#1F3B4D;'
                                'margin-bottom:6px">📅 Log Daily Quantity</div>',
                                unsafe_allow_html=True
                            )
                            _lc1, _lc2, _lc3 = st.columns([2, 2, 1])
                            _log_date = _lc1.date_input("Date", value=date.today(),
                                                         format="DD/MM/YYYY", key=f"{_bk}_logdate")
                            _log_qty  = _lc2.number_input("Quantity", min_value=0, value=0,
                                                           step=1, key=f"{_bk}_logqty")
                            _lc3.markdown('<div style="height:28px"></div>', unsafe_allow_html=True)
                            if _lc3.button("📥 Log", key=f"{_bk}_log", use_container_width=True):
                                if _bm_pid > 0:
                                    auth.upsert_bot_metric_log(_bm_pid, _bm_proj_name,
                                                               str(_log_date), _log_qty)
                                    auth.log_audit(cu["id"], cu["name"], "CREATE", "bot_metric_logs",
                                                   str(_bm_pid),
                                                   f'Qty {_log_qty} logged for "{_bm_proj_name}" on {_log_date}')
                                    st.session_state.toast = {"msg": f"Logged {_log_qty} for {_log_date}!", "type": "success"}
                                    st.rerun()

                            _proj_logs = auth.get_bot_metric_logs(project_id=_bm_pid)[:10]
                            if _proj_logs:
                                _log_disp = pd.DataFrame(_proj_logs)[["log_date", "qty"]].copy()
                                _log_disp.columns = ["Date", "Qty"]
                                st.dataframe(_log_disp, use_container_width=True, hide_index=True)
                        else:
                            _rv1, _rv2, _rv3, _rv4 = st.columns(4)
                            _rv1.metric("Persons",       int(float(_bm_r.get("num_persons",    0) or 0)))
                            _rv2.metric("Bots",          int(float(_bm_r.get("num_bots",        0) or 0)))
                            _rv3.metric("Manual (mins)", f"{float(_bm_r.get('manual_run_mins', 0) or 0):.0f}")
                            _rv4.metric("Bot (mins)",    f"{float(_bm_r.get('bot_run_mins',    0) or 0):.0f}")
                            _proj_logs_ro = auth.get_bot_metric_logs(project_id=_bm_pid)[:10]
                            if _proj_logs_ro:
                                _log_disp_ro = pd.DataFrame(_proj_logs_ro)[["log_date", "qty"]].copy()
                                _log_disp_ro.columns = ["Date", "Qty"]
                                st.dataframe(_log_disp_ro, use_container_width=True, hide_index=True)
        ...
        if not _licenses_all:
            ...
            with st.container(border=True):
                ...
                for _lic in _licenses_all:
                    _lr = st.columns([0.3, 2.2, 1.0, 1.3, 1.3, 1.4, 0.5, 0.5, 0.5], vertical_alignment="center")
            ...
            with st.container(border=True):
                ...
                for _slc, _sll in zip(_slhdr, ["#", "Tool Name", "Client", "Qty", "Start", "End", "Status", "Notes", "", "", ""]):
                    _slc.markdown(f'<div style="font-size:10px;font-weight:700;text-transform:uppercase;color:#64748B;'
                                  ...
                                  unsafe_allow_html=True)
                for _sl in _sold_licenses_all:
                    _slr = st.columns([0.3, 1.8, 1.8, 0.9, 1.2, 1.2, 1.2, 1.5, 0.5, 0.5, 0.5], vertical_alignment="center")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if role == "employee":
        ...
        def _render_task_dept_tabs():
            _tdept_rpa, _tdept_ws = st.tabs(["🔧 RPA", "⚙️ Worksoft"])
            with _tdept_rpa:
